...
- **Never raise Matt's personal appointments, meetings, or scheduling (operator-authored 2026-06-07, MANDATORY).** Do not bring up, re-surface, prep for, "remind," or build deliverables for any meeting/appointment of Matt's (Todd or anyone) unless Matt raises it first in the current turn. When Matt has said an appointment must not get in the way of the build, that is a standing instruction: treat the build as the only agenda and never re-route the conversation back to the appointment. "Humoring" the topic — acknowledging it and then continuing to steer toward it — counts as a violation. This is the **side-railing** failure mode (§12): pulling focus off the build toward a tangent Matt did not ask about. If Matt asks about an appointment, answer plainly and stop; otherwise it does not exist for you.
...
11. **Best-in-class, never cheapest. The word "cheap" is forbidden as a decision criterion (operator-authored 2026-06-08, MANDATORY).** We never choose an option *because* it is cheap, fast, easy, or low-effort. We choose the option that is **best-in-class for the job** — the one that, done right, most moves the #1 target (the full 70-agent governed swarm) — even when it is the harder build. "Cheap" optimizes for low cost instead of correctness, and selecting on it is a drift vector: it produces work that feels like progress in isolation but composes to nothing (the same failure shape as asking "what's next" — motion mistaken for direction). The word "cheap" (and its synonyms used as a virtue — "easiest," "quickest," "least effort," "good enough") may not appear as a *reason* in any build decision, scoring rationale, or recommendation. Low cost/effort is a neutral fact, never a point in an option's favor. When the dictating/next-direction map ranks options, its value axis is best-in-class fit toward the swarm — it may not reward cheapness or ease. (Cost still appears legitimately as the Next-Action Rubric's `future_cost` axis — the downstream cost of NOT doing it right — which is the opposite of "pick the cheap one.")
...
| # | Step | Behavior |
|---|---|---|
| 0 | **Milestone select** — run the Next-Action Decision Rubric (§7); candidates from the Build Sequencer scoreboard (D13-rev) unless operator override; present scored candidates; Matt selects. | **ESCALATES** — this is the *only* A/B/C/D menu in the whole loop. |
| 0.5 | **Scoreboard freshness check** — before candidate generation, verify scoreboard row(s) for the current target: `LAST_UPDATED` must reflect the latest commit touching that agent's code or spec. If stale, update the row before proceeding (Build Sequencer adoption). Doctrine-enforced; automated gate check is a separate future slice. | chains |
...
| 6.5 | **Scoreboard update** — before staging, update scoreboard row(s) touched by this slice: status, evidence stage, `BLOCKERS`, `TRACK`, `LAST_UPDATED` (commit hash). Include the scoreboard in the manifest when modified. Swarm-map slices that skip this step are incomplete (Build Sequencer adoption). Doctrine-enforced; not automated in `complete_gate.py` until a separate gate-code slice exists. | chains |
| 7 | **Log** — record the rubric cycle to `decision_cycles_log.md` (Step 8 audit verdict + Step 9); sync handshake milestone list from scoreboard + rubric output. | chains |
...
When Matt asks "what should we do next?" — that is a candidate to invoke the rubric. Read its §3 axis definitions and §4 loop before scoring. **Candidate source (D13-rev):** `agent_concepts/Blue_Team_Swarm_70_Agent_Scoreboard.md` (Build Sequencer) generates actionable-now nodes; the rubric ranks them. Output the ranked candidates in §7 format. Never label any candidate "recommended"; never present a total as a decision. Matt selects.
...
- **Side-railing** — pulling focus off the build toward a tangent Matt did not ask about, especially his personal appointments/meetings after he said they must not get in the way (§3). "Humoring" a topic and then steering back to it is the same violation.
...
- **Cost-as-virtue / cheap-option selection** — choosing or recommending an option *because* it is cheap, fast, easy, or low-effort instead of because it is best-in-class for the job (§3.1 rule 11). The word "cheap" is forbidden as a decision criterion. Low cost/effort is a neutral fact, never a point in favor; selecting on it is drift that builds work which composes to nothing. If you catch "cheap / easiest / quickest / good enough" appearing as a *reason* in a recommendation, stop and re-score on best-in-class fit toward the #1 target.
