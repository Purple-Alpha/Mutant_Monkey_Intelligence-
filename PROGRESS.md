...
Wave 2 candidate emitter (`score_sheet_candidate_emit.py` + `pre_ship_audit.py` wiring) is implemented and validated on Linux primary (`b7328e7`, backed up to `github` + `backup`). Wave 3 draft landed (`1b46ca3`). **Wave 3 §11 now operator-signed (Matt Nichol / Zebra-Comit, 2026-06-03):** the deep-dive walked its §10 open questions and resolved all eight into a new §10.A Operator-Confirmed Decisions block — Q2 canonical write surface = `Testing_Score_Sheet_Schema.md` amendment block with a §9.1 hard 100-row auto-split trigger to a dedicated ledger file; Q7 cryptographic proof explicitly rejected for v1 (in-band textual proof only); Q1/Q3/Q4/Q5/Q6/Q8 accepted as recommended (review helper authorized as future Wave 3.1 spec, free-text rejection reasons, track-slug table in Wave 0, deferred-TTL carry-forward, Manus datasets via separate `research_intake` spec, PII hook bundled with the Wave 3.1 review-script spec). §11 ratifies the spec + §10.A as governing Wave 3 truth and unlocks *drafting* the Wave 3.1 `review_ledger.py` + PII-hook spec, but authorizes **no code** until a separate operator "start build." Spec edits made on the Windows mirror; **pending: cp to `~/northstar` → gate → commit/push**. Next: optionally draft the Wave 3.1 `review_ledger.py` spec (spec only). No review-script or promotion tooling implemented until separately authorized.
