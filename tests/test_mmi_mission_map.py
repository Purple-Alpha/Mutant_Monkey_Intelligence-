...
class MissionMapTests(unittest.TestCase):
    ...
    def test_analyze_stage_b_b02_next_after_dec132(self):
        position = self.mod.analyze(self.mod._repo_root())
        self.assertTrue(position.active)
        self.assertEqual(position.stage_id, "stage_b")
        self.assertIsNotNone(position.next_waypoint)
        self.assertEqual(position.next_waypoint.waypoint_id, "b02")
        self.assertEqual(position.next_waypoint.hand_to, "Cursor")
    ...
    def test_render_stdout_envelope(self):
        ...
        self.assertIn("stage_id: stage_b", out)
        self.assertIn("next_waypoint:", out)
        self.assertIn("b02", out)
    ...
    def test_cli_position(self):
        ...
        self.assertIn("stage_id: stage_b", proc.stdout)
