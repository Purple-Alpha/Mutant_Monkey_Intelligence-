...
**Actionable-now (`BLOCKERS` empty, `TRACK=BREADTH`, dependencies satisfied):** **CYCLE 17 (2026-06-08) — DONE:** **#39 Language Pressure** §11-signed and built to `GOVERNED_AGENT` at Evidence Stage 1 — first agent selected by the LIVE Build Map. Governed wrapper `core/orchestrator/language_pressure_agent.py` (`LanguagePressureAgent`, committed `4987da6`, facts-only closed TOAD v1 category names from `body_plain` only via `detect_callback_phishing`, 18 focused tests incl. body_plain-only boundary + no risk-floor/phrase/phone leakage + purity guard). **Build Map TRIAGE (CYCLE 17):** **#25 Session Theft** marked **RECLASSIFY** — expected OAuth/session-token theft detector; actual cited file is `prompt_injection_detector.py` (LLM-protection agent, different concept). Re-triage trigger: standalone device-code/OAuth session-theft detector built (Frontier Intake Candidate 5) OR row corrected to a separate Prompt Injection agent entry. **#23**, **#24**, **#27**, **#30**, and **#39** are `GOVERNED_AGENT` awaiting Stage 2 promotion (depth gate CLOSED). **#31 PDF Fingerprint** remains `NEEDS_SIGNED_CONTRACT` (stateful Vendor Baseline Store mutation). **#44 Social Engineering** likely overlaps #39 (same detector surface — next triage should RECLASSIFY/merge). **#11 Known-Good Contact** is stateful / Layer 3. **#7 Sender Identity** needs a clean detector surface. **#18 Callback Verification** paused for boundary-contract split. Next action: run the next Build Map TRIAGE cycle on BREADTH to pull the next clean Layer 2 wrap.
