...
**BREADTH RUNWAY:** **7** of 70 agents at `GOVERNED_AGENT` (Evidence Stage 1 Synthetic): #6, #6A, #8, #10, #23, #27, #30.

**Actionable-now (`BLOCKERS` empty, `TRACK=BREADTH`, dependencies satisfied):** **CYCLE 15 (2026-06-08):** **#24 MFA Manipulation** selected (rubric 10) — pure facts-only `score_mfa_fatigue`; **Agent Design Contract DRAFTED, §11 signature pending** (`NEEDS_SIGNED_CONTRACT`). #24 is the sibling body-signal detector to #23 and is the cleanest immediate breadth wrap: no state, no network, closed indicators (`mfa_push_language`, `verification_code_language`). **#23 Credential Phishing**, **#27 Link Inspection**, and **#30 Attachment Risk** are `GOVERNED_AGENT` awaiting Stage 2 promotion (≥3 real samples + signed promotion record). **#31 PDF Fingerprint** remains blocked as `NEEDS_SIGNED_CONTRACT` because it mutates the per-tenant Vendor Baseline Store. Remaining clean pure-detector candidate after #24: **#25 Session Theft** (`prompt_injection_detector.py`). **#11 Known-Good Contact** is stateful / Layer 3 (tighter boundary contract needed). **#7 Sender Identity** needs a clean detector surface (current evidence is a broad LLM scoring field). **#18 Callback Verification** is paused for a contract split decision. Spine slices (e.g. second Layer 5 Challenge agent) are session-level candidates when not tied to a single row.
...
| 24 | MFA Manipulation | `DETECTOR_FUNCTION` | `core/precursor/body_signal_detector.py` (`score_mfa_fatigue`; pure facts-only, no state — clean wrap candidate; **Agent Design Contract DRAFTED 2026-06-08 `4. Product_Roadmap/MFA_Manipulation_Agent_Design_Contract_Deep_Dive.md`, §11 signature pending**) | 2 Detection | A | NEEDS_SIGNED_CONTRACT | BREADTH | 10 | cycle15 |
...
| 31 | PDF Fingerprint | `DETECTOR_FUNCTION` | `core/scoring/document_metadata_detector.py` (`assess_document_metadata_fingerprint` MUTATES the per-tenant Vendor Baseline Store via check_signal + ingest_signal and needs tenant_id/vendor_domain/now — stateful Layer 2/3 boundary; needs a signed boundary contract before a clean wrap, like #11/#18) | 2 Detection | A | NEEDS_SIGNED_CONTRACT | BREADTH | — | cycle14 |
...
- `GOVERNED_AGENT`: **7** (#10; #6 Header Analysis, #8 Ghost Thread, and #6A Email Authentication at Evidence Stage 1, §11-signed 2026-06-07; #27 Link Inspection, #30 Attachment Risk, and #23 Credential Phishing at Evidence Stage 1, §11-signed 2026-06-08).
- `DETECTOR_FUNCTION`: **~23** (the scoring/precursor/vendor-baseline/evidence/sandbox/blackboard surfaces; #6 and #8 promoted out of this bucket on 2026-06-07, #27, #30, and #23 on 2026-06-08; #31 PDF Fingerprint now carries NEEDS_SIGNED_CONTRACT for a stateful Layer 2/3 boundary).
...
**Headline:** the swarm is currently a **detector stack plus a partially wired governance spine, not yet a governed swarm**. Seven agents (#10 Lookalike, #6 Header Analysis, #8 Ghost Thread, #6A Email Authentication, #27 Link Inspection, #30 Attachment Risk, #23 Credential Phishing) are now governed agents at Evidence Stage 1 Synthetic; everything else that "exists" is either a detector function inside the scoring pipeline or a spine component that still lacks a formal per-agent contract. The orchestration spine now has a router + registry + Stage A case loop + in-memory Decision Evidence Record assembly + Layer 5 challenge pass, proven against real Header Divergence, Ghost Thread, and Email Authentication contributions (three detector wrappers on one proven path). The remaining gap is wiring the Layer 5 challenge pass against the aggregate contribution set from multiple agents in one case, then formal promotion of more real agents through signed Agent Design Contract wrappers.
