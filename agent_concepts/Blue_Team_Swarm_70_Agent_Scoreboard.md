...
**Actionable-now (`BLOCKERS` empty, `TRACK=BREADTH`, dependencies satisfied):** **CYCLE 14 CLOSED (2026-06-08):** **#23 Credential Phishing** is now `GOVERNED_AGENT` (Evidence Stage 1, §11-signed 2026-06-08, wrapper `75affc6`, 17 tests); its next gate is the Stage 2 promotion bar (≥3 real samples + signed promotion record). **#31 PDF Fingerprint is NOT a clean wrap** (now `NEEDS_SIGNED_CONTRACT`): `assess_document_metadata_fingerprint` calls `vendor_baseline.check_signal` AND `ingest_signal`, so running it mutates the per-tenant Vendor Baseline Store and writes audit records — a stateful Layer 2/Layer 3 boundary problem (same class as #11/#18), needs a boundary contract before code. **#27 Link Inspection** and **#30 Attachment Risk** are likewise `GOVERNED_AGENT` awaiting Stage 2. Remaining clean pure-detector candidates with no state: **#24 MFA Manipulation** (`score_mfa_fatigue`, sibling of #23 — cleanest immediate next wrap) and **#25 Session Theft** (`prompt_injection_detector.py`). **#11 Known-Good Contact** is stateful / Layer 3 (tighter boundary contract needed). **#7 Sender Identity** needs a clean detector surface (current evidence is a broad LLM scoring field). **#18 Callback Verification** is paused for a contract split decision. Spine slices (e.g. second Layer 5 Challenge agent) are session-level candidates when not tied to a single row.
