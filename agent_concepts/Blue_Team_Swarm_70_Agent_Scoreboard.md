...
**Actionable-now (`BLOCKERS` empty, `TRACK=BREADTH`, dependencies satisfied):** **CYCLE 14 (2026-06-08):** **#23 Credential Phishing** selected (rubric 10) — pure facts-only `score_credential_harvesting`; **Agent Design Contract DRAFTED, §11 signature pending** (`NEEDS_SIGNED_CONTRACT`). **#31 PDF Fingerprint is NOT a clean wrap:** `assess_document_metadata_fingerprint` calls `vendor_baseline.check_signal` AND `ingest_signal`, so running it mutates the per-tenant Vendor Baseline Store and writes audit records — a stateful Layer 2/Layer 3 boundary problem (same class as #11/#18), needs a boundary contract before code. **#30 Attachment Risk** and **#27 Link Inspection** are `GOVERNED_AGENT` (Evidence Stage 1, §11-signed 2026-06-08); next gate is the Stage 2 promotion bar (≥3 real samples + signed promotion record). Other clean pure-detector candidates with no state: **#24 MFA Manipulation** (`score_mfa_fatigue`, sibling of #23) and **#25 Session Theft** (`prompt_injection_detector.py`). **#11 Known-Good Contact** is stateful / Layer 3 (tighter boundary contract needed). **#7 Sender Identity** needs a clean detector surface (current evidence is a broad LLM scoring field). **#18 Callback Verification** is paused for a contract split decision. Spine slices (e.g. second Layer 5 Challenge agent) are session-level candidates when not tied to a single row.
...
| # | SPARK agent | Runtime status | Code evidence | Canonical 6-layer (pinned) | Stage | BLOCKERS | TRACK | LAST_RUBRIC_SCORE | LAST_UPDATED |
|---|---|---|---|---|---|---|---|---|---|
| 23 | Credential Phishing | `DETECTOR_FUNCTION` | `core/precursor/body_signal_detector.py` (`score_credential_harvesting`; pure facts-only, no state — clean wrap candidate; **Agent Design Contract DRAFTED 2026-06-08 `4. Product_Roadmap/Credential_Phishing_Agent_Design_Contract_Deep_Dive.md`, §11 signature pending**) | 2 Detection | A | NEEDS_SIGNED_CONTRACT | BREADTH | 10 | cycle14 |
