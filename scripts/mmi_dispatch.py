    ...
    if not tasks:
        ...
        lines += [
            ...
            (
                ...
                "YES — supply next evidence or run Estimator for read-only candidate ranking",
            ),
            (
                "CANDIDATES_NOT_AUTHORIZATION",
                "YES — Estimator ranks candidates; output is not authorization, selection, or routing authority",
            ),
            ("REQUIRED_UPDATE_AFTER_COMPLETION", WORKER_COMPLETION_UPDATE),
            (
                "NEXT_GATE",
                "new signed contract, scoreboard row, intake evidence, or operator-selected next action",
            ),
...
def run_doctrine_checks():
    ...
    checks.append((
        mode in ("DELEGATE", "ALL_CLEAR")
        and (
            (
                mode == "DELEGATE"
                and "CANDIDATES_NOT_AUTHORIZATION" in delegate
                and "TASK_SCOREBOARD" in delegate
                and "NEXT_DELEGATED_TASK" in delegate
            )
            or (
                mode == "ALL_CLEAR"
                and "WHY_QUEUE_IS_EMPTY" in delegate
                and "CANDIDATES_NOT_AUTHORIZATION" in delegate
                and delegate.get("BUILD_AUTHORIZATION_IMPLIED") == "NO"
            )
        ),
        "DELEGATE or ALL_CLEAR emits queue-empty / not-authorization guard",
    ))
    ...
    checks.append((
        ...
        and "WHY_QUEUE_IS_EMPTY" in dispatch_src
