"""Evidence Stage 1 proof for swarm agent #18 Callback Verification.

Authorized by the §11-SIGNED Callback Verification Agent Design Contract
(2026-06-25). These synthetic tests prove the Layer 3 Verification wrapper is a
read-only callback-phishing OOB posture projector: workflow-state mapping, no
TOAD detector calls, no workflow writes, tenant isolation, persistence,
registry-default exclusion, no network/subprocess behavior, and no raw leakage.
"""
...
from core.orchestrator import callback_verification_agent as cva
from core.orchestrator.callback_verification_agent import (
...
TENANT = "tenant_callback_verification_demo"
FINDING_ID = "callback_phishing:case-001"
DETECTOR = "callback_phishing_detector_001"
NOW = datetime(2026, 6, 25, 12, 0, tzinfo=timezone.utc)

_ALLOWED_FACT_PREFIXES = frozenset(
    {
        "callback_oob_verification_missing",
        "callback_oob_verification_pending",
        "callback_oob_verification_confirmed",
        "callback_oob_verification_rejected",
        "callback_oob_verification_unable_to_verify",
        "callback_oob_verification_expired",
        "callback_verification_detector:",
    }
)
...
def _verification_entry() -> AgentRegistryEntry:
    return AgentRegistryEntry(
        agent_id=CALLBACK_VERIFICATION_AGENT_ID,
        display_name="Callback Verification Agent",
...
def _context(*, tenant_id: str = TENANT, digest: str | None = None) -> MissionContext:
    return MissionContext(
        ...
        or digest_request(
            ...
            verification_class=CALLBACK_PHISHING_OOB_CLASS,
        ),
    )
...
def _agent(
    *,
    ...
    return CallbackVerificationAgent(
        finding_id=finding_id,
...
def _run(agent: CallbackVerificationAgent, *, tenant_id: str = TENANT):
...
def test_callback_verification_agent_satisfies_agent_protocol():
...
def test_missing_workflow_record_emits_unable_to_verify():
    ...
    assert contribution.agent_id == CALLBACK_VERIFICATION_AGENT_ID
    assert contribution.layer == 3
    assert contribution.observed_facts == ("callback_oob_verification_missing",)
    assert contribution.verification_source == f"{CALLBACK_PHISHING_OOB_CLASS}:{FINDING_ID}"
...
def test_pending_confirmation_emits_unable_to_verify():
    ...
    assert contribution.observed_facts == (
        "callback_oob_verification_pending",
        f"callback_verification_detector:{DETECTOR}",
    )
    assert contribution.verification_outcome == "unable_to_verify"
...
def test_wrapper_never_calls_workflow_write_paths(monkeypatch):
    ...
    def fail_write(*args, **kwargs):
        raise AssertionError("CallbackVerificationAgent must not write workflow state")
...
def test_invalid_verification_class_fails_closed():
    with pytest.raises(GovernanceError, match="verification_class"):
        CallbackVerificationAgent(
            finding_id=FINDING_ID,
            blackboard_root=Path("blackboard"),
            verification_class="other_class",
        )
...
def test_tenant_isolation_keeps_confirmation_state_separate():
    _seed_outcome("confirmed", tenant_id="tenant_callback_a")

    confirmed = _run(_agent(), tenant_id="tenant_callback_a").contributions[0]
    missing = _run(_agent(), tenant_id="tenant_callback_b").contributions[0]

    assert confirmed.verification_outcome == "confirmed"
    assert confirmed.observed_facts[0] == "callback_oob_verification_confirmed"
    assert missing.verification_outcome == "unable_to_verify"
    assert missing.observed_facts == ("callback_oob_verification_missing",)
...
def test_read_only_summary_remains_available_under_production_kill_switch():
    ...
    contribution = _run(_agent()).contributions[0]
    assert contribution.observed_facts[0] == "callback_oob_verification_pending"
    assert contribution.verification_outcome == "unable_to_verify"
...
def test_contribution_persists_to_blackboard_and_reads_back():
    ...
    assert payload_back.agent_id == CALLBACK_VERIFICATION_AGENT_ID
    assert payload_back.case_id == context.case_id
    assert payload_back.inputs_digest == context.inputs_digest
    assert payload_back.verification_source == f"{CALLBACK_PHISHING_OOB_CLASS}:{FINDING_ID}"
...
def test_unauthorized_agent_cannot_write_contribution():
    ...
    payload = AgentContributionPayload(
        ...
        agent_id="ingest_001",
        layer=3,
        observed_facts=["callback_oob_verification_confirmed"],
        verification_source=f"{CALLBACK_PHISHING_OOB_CLASS}:{FINDING_ID}",
...
def test_callback_verification_not_in_default_registry():
    assert CALLBACK_VERIFICATION_AGENT_ID not in build_default_registry()
...
def test_wrapper_performs_no_network_or_subprocess(monkeypatch):
    ...
    def no_network(*args, **kwargs):
        raise AssertionError("CallbackVerificationAgent must not open a socket")

    def no_subprocess(*args, **kwargs):
        raise AssertionError("CallbackVerificationAgent must not spawn a subprocess")
    ...
    source = inspect.getsource(cva.CallbackVerificationAgent)
...
def test_wrapper_never_calls_toad_detector_or_workflow_writes():
    wrapper_source = inspect.getsource(cva.CallbackVerificationAgent)

    for forbidden in (
        "detect_callback_phishing",
        "callback_phishing_detector",
