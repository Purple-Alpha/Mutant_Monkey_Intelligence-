"""End-to-end proof that the governed-agent contract holds on ghost-thread output."""
...
from core.orchestrator.ghost_thread_agent import (
    GHOST_THREAD_AGENT_ID,
    GhostThreadAgent,
    digest_email,
)
...
_CHALLENGE_AGENT_ID = "ghost_thread_challenge_001"


def _ghost_agent_entry() -> AgentRegistryEntry:
    return AgentRegistryEntry(
        agent_id=GHOST_THREAD_AGENT_ID,
        display_name="Ghost Thread Agent",
...
def _challenge_entry() -> AgentRegistryEntry:
    return AgentRegistryEntry(
        ...
        display_name="Ghost Thread Challenge Agent",
        role=AgentRole.BLUE,
        allowed_environments={Environment.PRODUCTION, Environment.SANDBOX},
...
def _registry() -> dict[str, AgentRegistryEntry]:
    return {
        e.agent_id: e
        for e in (_ghost_agent_entry(), _ingest_entry(), _challenge_entry())
    }
...
def _agent(route_ctx: RouteContext) -> GhostThreadAgent:
    return GhostThreadAgent(
...
class _ChallengeAgent:
    ...
    def challenge(
        ...
        if not any(
            "ghost_thread_subject" in c.observed_facts for c in contributions
        ):
            return None
        return ChallengeResult(
            agent_id=self.agent_id,
            challenge_outcome="confirmed",
            challenge_basis="Ghost-thread subject fact is structurally present.",
        )


def test_ghost_thread_agent_satisfies_agent_protocol(tmp_path):
    assert isinstance(_agent(_route_ctx(tmp_path)), Agent)


def test_known_bad_email_produces_ghost_thread_contribution(tmp_path):
    ...
    assert contribution.agent_id == GHOST_THREAD_AGENT_ID
    assert contribution.layer == 2
    assert contribution.observed_facts == ("ghost_thread_subject",)
    assert der.disposition == "suspicious"
    assert der.inputs_digest == digest_email(payload)
    assert len(der.inputs_digest) == 64


def test_known_good_threaded_reply_produces_empty_facts_and_clear(tmp_path):
    route_ctx = _route_ctx(tmp_path)
    record_id, payload = _seed_email(
        route_ctx,
        subject="Re: Vendor invoice approval",
...
def test_contribution_persists_to_blackboard_and_reads_back(tmp_path):
    ...
    assert write.record.source_agent == GHOST_THREAD_AGENT_ID
    ...
    assert payload_back.agent_id == GHOST_THREAD_AGENT_ID
    assert payload_back.case_id == context.case_id
    assert payload_back.inputs_digest == context.inputs_digest
    assert payload_back.observed_facts == ["ghost_thread_subject"]


def test_layer5_challenge_pass_runs_against_ghost_thread_contribution(tmp_path):
    route_ctx = _route_ctx(tmp_path)
    record_id, payload = _seed_email(
        route_ctx,
        subject="Re: Vendor invoice approval",
    ...
    assert der.contributions[0].observed_facts == ("ghost_thread_subject",)
...
def test_unauthorized_agent_cannot_write_contribution(tmp_path):
    ...
    payload = AgentContributionPayload(
        ...
        agent_id=_INGEST_AGENT_ID,
        layer=2,
        observed_facts=["ghost_thread_subject"],
    )
...
def test_agent_contribution_payload_round_trips_via_payload_models():
    record = BlackboardRecord(
        ...
        source_agent=GHOST_THREAD_AGENT_ID,
        payload=AgentContributionPayload(
            case_id=uuid4(),
            inputs_digest="a" * 64,
            agent_id=GHOST_THREAD_AGENT_ID,
    ...
    assert parsed.observed_facts == ["ghost_thread_subject"]
