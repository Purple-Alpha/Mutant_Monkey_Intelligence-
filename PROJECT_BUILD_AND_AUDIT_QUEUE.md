...
1. **Real-customer-data controls decision (gated).** Before any non-synthetic package is ever submitted to the stage-9 Grok audit OR rendered for buyer delivery, a separate controls/redaction-for-external-audit decision + spec is required. v1 is synthetic/test packages only.
