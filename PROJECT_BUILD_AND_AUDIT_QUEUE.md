...
1. **Real-customer-data controls spec: §11 SIGNED 2026-06-05 (Matt Nichol; committed `e77f85c`).** Consequence Matrix outcome was Option B; D1-D15 locked. Signing locks the controls contract only — it does NOT change §13/IQ3, authorize real-data handling, open buyer delivery, or start infrastructure. Next buildable items behind the signed contract (all gated, none auto-started): Production Evidence Store deep-dive (D12), §13/IQ3 revision path (D7), NorthStar local-AI package-audit brief (D11).
