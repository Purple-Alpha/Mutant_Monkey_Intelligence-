...
1. **Real-customer-data controls spec: §11 SIGNED 2026-06-05 (Matt Nichol; committed `e77f85c`).** Consequence Matrix outcome was Option B; D1-D15 locked. Signing locks the controls contract only — it does NOT change §13/IQ3, authorize real-data handling, open buyer delivery, or start infrastructure. Progress behind the signed contract: D11 package-audit brief authored (`6026deb`, `Mutant_Monkey_Package_Audit_Brief.md`, pre-deployment); Production Evidence Store deep-dive DRAFT pre-§11 (`b449593`). Still gated/unstarted: §13/IQ3 revision path (D7, butterfly), local-AI substrate + D9 calibration, Production Evidence Store §11, buyer delivery (D8/D15).
2. **Buyer PDF delivery (gated).** The renderer exists for internal/synthetic packages only. Actual buyer-facing PDF delivery is excluded by the standing Pass-1 authorization and stays an explicit operator decision (ties to item 1 and any later production-delivery spec).
3. **Production Evidence Store spec (after §11, before real data).** Real customer artifacts require a separate production datastore spec per proposed D12 before any real package is generated, stored, audited, rendered, or delivered.
4. **Local-AI audit substrate build (after §11 + explicit start-build).** Dax-with-NorthStar-audit-brief is the proposed default (D11), but no local model/runtime/hardening implementation begins until the controls spec is signed and Matt gives the start-build instruction.
5. **Actual package-level operator signature (when a concrete package is ready).** The mechanism exists; Matt must review the rendered package and supply his own wording. No AI-authored sign-off text.
6. **Push remains an explicit operator instruction.** Pushing is never inferred; operator pushed GitHub through `f1fb901` on 2026-06-05.
