...
**Authority:** Signed by Matt Nichol June 13th 2026 (§18) — build authorization granted per this contract's scope. Signing does not by itself change any scoreboard row, deploy code, or rewrite historical gate records.
**Relationship to prior contract:** This v2 contract supersedes `Specialisation_Fission_Contract.md` going forward, effective on signing (Matt Nichol June 13th 2026). Local repo evidence shows scoreboard row #91 as `GATED` from the prior 2026-06-12 build; this supersession does not rewrite that historical gate record.
...
This contract is signed (§18). Implementation of the v2 scope is authorized and must still satisfy every requirement below before closure.
