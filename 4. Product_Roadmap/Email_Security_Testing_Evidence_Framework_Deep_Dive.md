...
- **Q6.** Composite quality score on the dashboard, or strictly the §8.5 metric list? Spec recommends "no composite in v1" but flags this for stress test.
- **Q7.** Decision Auditor (`audit_tools/decision_audit_runner.py`) integration shape — likely yes (failure cards are decision-audit candidates), but undefined.
