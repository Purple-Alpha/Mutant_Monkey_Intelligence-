...
**Status:** Operational plan v1 (pre-deployment); §9 open questions operator-resolved 2026-06-06 (see §9.A, lean-first / evidence-triggered hybrid). **2026-06-07: minor lean refinements added after two independent external overviews — baseline source-version/immutability fields (§9.A Q1, §6), and a §3 coverage precondition in §4; the Next-Action Decision Rubric was reviewed and left unchanged.** Authored 2026-06-06 by Cursor on Matt Nichol's instruction. This plan operationalizes the **D9 calibration gate** defined in `4. Product_Roadmap/Mutant_Monkey_Package_Audit_Brief.md` §6 and locked by `4. Product_Roadmap/Real_Customer_Data_Controls_Deep_Dive.md` D9. It is a runnable calibration procedure, not a signed spec; it edits no signed spec and locks no new decision.
...
- **Coverage precondition (checked first).** The run's `corpus_manifest` must cover **every §3 planted-defect catalog row and every §3 hard boundary at least once**. A run that omits any catalog row cannot return `calibration_pass`; it is a `calibration_miss` with reason "incomplete coverage." This makes the §9.A Q2 "every catalog row has a case" rule a hard gate, not an aspiration (2026-06-07 review refinement) — and it is the explicit check that must pass before any decision to defer corpus expansion.
...
baseline_source_version:    # echo of baseline_reference.json stage9_source_version + captured_at_utc (traceability, per 2026-06-07 review)
...
1. **Q1 — Frozen baseline reference.** The R1 reference is a **single `baseline_reference.json` versioned in git**, not a live Grok call and not handshake front-matter. It is captured once during the Grok-available window: per known-good package, its `package_input_sha256` and the recorded Stage-9 `clean` verdict. R1 compares the local verdict to that frozen record (snapshot-style: compare to stored reference, fail on mismatch). Deterministic, offline, zero token cost, no provider-uptime dependency. Re-baselining is a deliberate, reviewed change, not an automatic overwrite. For traceability (2026-06-07 external review), `baseline_reference.json` carries two in-file fields: `stage9_source_version` (the Stage-9 ruleset identity the verdicts were captured against) and `captured_at_utc`. The file is documented **immutable in place** — it is superseded only by a new versioned re-baseline (bump `stage9_source_version`), never silently edited, so any reference change is visible in git history.
