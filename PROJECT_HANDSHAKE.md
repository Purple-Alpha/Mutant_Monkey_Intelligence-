...
- **Mutant Monkey package-auditor D9 calibration plan DRAFTED (2026-06-06; committed `581b7f3`).** `4. Product_Roadmap/Mutant_Monkey_Package_Auditor_D9_Calibration_Plan.md` operationalizes the D9 gate (brief §6 + controls D9): synthetic-only corpus (known-good / planted-defect / refuse), three conjunctive requirements, a six-row planted-defect catalog mapped to expected blocking findings + contract refs, a 100% conjunctive pass bar (false pass = dangerous miss), a calibration-run record schema, and re-calibration triggers. Synthetic/test only; no signed-spec edit, no infrastructure, no real data; a `calibration_pass` still needs controls D7 + substrate + explicit operator activation. Four operator-only open questions (§9) remain. Gate clean 0/0: `audit_outputs/mutant_monkey_d9_calibration_plan_20260607_20260607T012634Z.md`.
...
no real data; gate clean 0/0). Four operator-only open questions remain in §9.
