...
**ACTIVE TRACK: the full 70-agent swarm build (Matt's #1 target).** The next swarm step is Q5 step 1 from the scoreboard — define the **Decision Evidence Record (DER) type + the shared agent interface** that every governed agent is built against. That is a design artifact, so per AGENTS §2.1.2 it routes to Claude (design lane) first; the execution lane drafts against the locked design after. Build order then proceeds spine-first: DER/interface -> #1 Swarm Commander case loop + #2 Mission Context -> #10/#21 metadata retrofit -> wrap existing detectors -> net-new detectors -> Stage B (signed-authorization-gated).
...
**70-agent scoreboard v1 BUILT (2026-06-07, committed `d832b3c`):** `agent_concepts/Blue_Team_Swarm_70_Agent_Scoreboard.md` reconciles the SPARK 70-agent inventory + 6-layer Design Tree against the live runtime. Execution-lane verified status: 1 GOVERNED_AGENT (#10), ~28 DETECTOR_FUNCTIONs, 7 GOVERNANCE_DOC_ONLY, 7 SPEC_ONLY, ~22 NOT_STARTED. Headline: the swarm is a detector stack, not a governed swarm; the orchestration spine (#1 Commander case loop, #2 Mission Context, Decision Evidence Record assembly) is the critical gap. **Five reconciliation questions RESOLVED 2026-06-07 (committed `3db395f`):** Claude (design lane) pinned canonical 6-layer placement for the ambiguous agents, canonical names, SPARK-only-agent scope (default-in-scope, blockers tagged not excluded), the per-layer promotion bar that defines "done = governed agent," and the spine-first build order. All folded into the scoreboard. No agent build is authorized by the scoreboard (Rule 4); the per-agent Rubric -> spec -> gate -> sign path still governs each promotion. **Next swarm step:** the DER type + shared agent interface design (routes to Claude first per §2.1.2).
