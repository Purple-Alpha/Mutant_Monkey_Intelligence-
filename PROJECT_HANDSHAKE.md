...
**Real-customer-data controls spec is §11 SIGNED (2026-06-05, Matt Nichol; committed `e77f85c`).** The controls path is Option B: real customer packages audited by operator-controlled local AI on a locked machine; Grok/xAI stays synthetic/test only until sunset; Private Test-Data Store stays test/lab forever. Locks D1-D15 (D9 calibration gate over fixed model, D10 locked-machine controls, D11 Dax-with-NorthStar-audit-brief default + builder/auditor separation, D12 separate Production Evidence Store spec, D13 Stage-9-parity local audit output, D14 Grok/xAI synthetic sunset triggers, D15 buyer delivery separately gated, plus D1-D8). **Signing locks the contract only — it authorizes nothing operational.** No infrastructure, no §13/IQ3 revision, no real-customer-data handling, and no buyer delivery begin without a separate explicit operator start-build/sign-off. Buildable items behind the signed contract (all gated, none started): Production Evidence Store deep-dive (D12), §13/IQ3 revision path (D7), NorthStar local-AI package-audit brief (D11). Next live action is operator's call (next milestone). PDF render surface is COMPLETE and synthetic/internal-only; buyer-brand boundary revision is complete/re-signed with **Mutant Monkey Inbox Shield** on package surfaces. The Cyber Insurance generator has stages 8/9/10, criterion-14 signature evidence, and PDF renderer wired/proven for synthetic packages. Rebrand Option B remains in force: Mutant Monkey Security external brand; NorthStar/SwarmCommand internal codenames; deep rename deferred. Build loop canonical (AGENTS §3.2); STANDING in force.
...
## Last Updated
2026-06-05 - Updated after real-customer-data controls spec §11 SIGNED (committed `e77f85c`, gates clean
0/0). Contract D1-D15 locked; signing authorizes nothing operational. GitHub push remains operator's
explicit terminal step (run `git status -sb` for the live delta).
