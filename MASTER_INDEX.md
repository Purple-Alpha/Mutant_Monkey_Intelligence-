...
- 4. Product_Roadmap/Private_Test_Data_Store_Deep_Dive.md - **DRAFT (pre-§11), authored 2026-06-04 by Cursor (Claude Opus 4.8) on Matt Nichol's instruction (milestone C); no infrastructure stood up; §11 signature blank by design.** Spec-first contract for an operator-controlled, self-hosted store for NorthStar test data + generated evidence artifacts (synthetic `.eml` corpora, evidence packages, audit packets, reaction-timing artifacts), so sensitive test material never transits third-party AI / consumer-cloud. Proposes D1-D8: self-hosted operator-controlled (D1), S3-compatible MinIO interface chosen over Nextcloud for programmatic object storage (D2), private-mesh-only no public ingress (D3), purpose-+tenant-scoped buckets mirroring runtime isolation (D4), synthetic/lab data only inheriting the Email Security Testing boundary (D5), encryption + no secrets in repo (D6), versioning + tested restore path (D7), no third-party processing by default (D8). §10 leaves seven operator-only questions (host target, MinIO topology, mesh sovereignty tradeoff Tailscale-vs-WireGuard, retention, integration surface, key management, production boundary). Holds large/binary/regenerable artifacts that do not belong in git; git stays source of truth for code/specs. Implementation does not begin until §11 + a separate start-build instruction.
