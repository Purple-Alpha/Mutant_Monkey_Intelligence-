...
- agent_concepts/Mutant_Monkey_Blue_Team_Swarm_Design_Tree.md - **SPARK / design capture (v2), NOT a spec, NOT §11, authority-free (created 2026-06-05 by Cursor, capturing Matt Nichol's operator design verbatim).** The governed evolution of the adopted v1 swarm map: adds a command layer (Swarm Commander / Mission Context / Severity Commander / Human-in-the-Loop / Final Review), Rules of Engagement, six agent authority levels, a domain-specific agent reputation (promotion/demotion) system, a two-pass detect-then-challenge decision model that replaces exposed chain-of-thought, a client-safe decision evidence record (observed facts / interpretations / assumptions / missing evidence / recommended verification / final outcome), conflict resolution, nine specialist teams, and a concrete 14-agent V1 starting swarm. Builds/authorizes nothing, expands no signed scope; agent names, positioning lines, and slogans are internal design labels NOT approved buyer claims (any buyer-heard wording still passes the `Compliance_and_Trend_Watch_Process.md` §5 claim boundary). Each agent promotes only via Next-Action Rubric -> spec-first -> gate -> sign-off; the V1 14-agent list is the natural draw order. Promoting v2 to canonical (superseding v1) is a one-line operator call not yet made.
- agent_concepts/_Blue_Team_Swarm_Architecture_Map_SPARK.md - **SPARK / vision capture, NOT a spec, NOT §11, authority-free; OPERATOR-ADOPTED 2026-06-05 as the official Stage B/C architecture map (adoption is not build authorization).** (Created 2026-06-05 by Cursor, capturing Matt Nichol's operator vision verbatim; moved from 4. Product_Roadmap into agent_concepts/ on 2026-06-05.) A 70-agent / ten-team blue-team swarm architecture for email fraud, phishing, ransomware precursors, vendor-payment deception, and cyber-insurance evidence — the detailed articulation of `VISION.md` Stage B (auto-defend obvious / escalate ambiguous) and Stage C (self-evolving swarm). Builds nothing, authorizes nothing, expands no signed scope; does not override AGENTS.md, the seven non-negotiables, or any signed spec. Flags the work as a butterfly / path-setting artifact (each agent promotion runs the normal Next-Action Rubric -> spec-first -> gate -> sign-off path; Consequence Matrix available), holds Stage discipline (no autonomous action authorized; Stage A stays analyze+recommend; real data stays behind the signed Real-Customer-Data Controls contract), keeps the insurance-named agents inside the `Compliance_and_Trend_Watch_Process.md` §5 carve-outs, and includes a cross-map tagging each of the 70 agents [EXISTS]/[SPECCED]/[GOVERNANCE]/[NET-NEW] against current runtime + signed/drafted specs (~30-40 already have a surface; the clear net-new cluster is the orchestration layer #1/#2/#68/#69 plus several Stage-B detectors). Serves as the backlog source that daily milestones are drawn from per `DECISION_PROTOCOL.md` §4.
