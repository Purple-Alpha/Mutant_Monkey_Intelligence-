...
- 4. Product_Roadmap/Link_Inspection_Agent_Design_Contract_Deep_Dive.md - **DRAFT 2026-06-08; not §11 signed; no runtime code authorized.** Candidate per-agent Agent Design Contract for swarm #27 Link Inspection. Draft boundary: future `LinkInspectionAgent` wraps immutable `score_url_obfuscation` (`core/precursor/url_obfuscation_detector.py`) as a Layer 2 Detection / Authority L3 / Stage A / Evidence Stage 1 Synthetic wrapper; emits facts-only URL indicator names as `observed_facts`; no numeric URL-obfuscation score, raw URL, body snippet, network/DNS/URL fetch, default-registry registration, production dispatch, scoring/rubric change, or autonomous action. §11 signature remains operator-only before implementation.
