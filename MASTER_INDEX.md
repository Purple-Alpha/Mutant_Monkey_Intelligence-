...
- 4. Product_Roadmap/Real_Customer_Data_Controls_Deep_Dive.md - **DRAFT (pre-§11) controls contract, authored 2026-06-05, rolling forward from the real-customer-data controls decision (Option B).** Defines the boundary between the synthetic/test audit path (external model permitted) and the real-customer audit path (operator-controlled local AI on a locked machine, no external egress). Proposed locked decisions D1-D15; §10 resolved 2026-06-05 by Matt's "agree all" instruction (D9 calibration gate over fixed model, D10 locked-machine controls, D11 Dax-with-NorthStar-audit-brief default, D12 separate Production Evidence Store spec, D13 Stage-9-parity local audit output, D14 Grok/xAI synthetic sunset triggers, D15 buyer delivery separately gated); §11 blank. Authorizes no infrastructure, no §13/IQ3 change, and no real-customer-data handling by itself.
