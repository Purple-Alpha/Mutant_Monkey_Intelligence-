...
- Cyber_Insurance_Vendor_Payment_Integrity_Discovery_Call_Sheet.md - **RETIRED / REMOVED FROM ACTIVE WORKING TREE 2026-06-02 by operator direction.** Former operator-facing target/contact sheet for first Cyber Insurance / Vendor Payment Integrity discovery attempts. Matt removed the call-sheet path as an active build blocker after concluding the project has enough research / Reddit / Todd / live-run / testing-footing signal to continue evidence-package build shaping without cycling on this worksheet. Historical references may remain in prior logs. Removal does not claim D10 completion, does not authorize §13 sign-off, does not authorize implementation, and does not create client-facing claims.
...
- `Research_Inputs/Testing_Score_Sheet_Schema.md` - **Pre-spec shaping artifact. Research input only — not §11, not §13, not D10 evidence, not implementation, not runtime code, not client-facing copy, not pricing approval, not an underwriter / broker / insurer claim.** Authored 2026-06-01 at operator request to establish the footing for NorthStar's internal testing discipline: a 12-column row schema for meaningful test events (`test_id`, `event_type`, `what_was_tested`, `expected_outcome`, `actual_outcome`, `pass_fail`, `why_plain_english`, nullable `failure_type`, nullable `corrective_action`, nullable `retest_evidence`, `recorded_at`, `recorded_by`). Captures rules for pass / fail / partial / blocked rows; preserves false positives, false negatives, blocked tests, and calibration observations; requires failure rows to link to correction evidence or an explicit open correction state; forbids weakening tests to make failures disappear; includes seed rows for the 2026-05-22 vendor-invoice false-negative recovery and the 2026-06-01 live `grok-4` auth-pass / content-risk calibration run. Includes an internal / external boundary and a forward-link to a future agent-bible spine without creating that artifact tonight.
- `Research_Inputs/Testing_Plan_V1.md` - **Pre-spec testing plan. Research / shaping input only — not §11, not §13, not D10 evidence, not implementation, not runtime code, not client-facing copy, not pricing approval.** Authored 2026-06-01 after `Testing_Score_Sheet_Schema.md` to define the first bounded set of test events to record against the score-sheet schema: the 2026-05-22 Phase 1.5 eval failure, the post-correction `vf-001` through `vf-005` diagnostics, the 2026-06-01 blocked missing-`openai` dependency attempt, the successful live `grok-4` auth-pass / content-risk calibration run, and an optional future full eval rerun hook. Locks recording rules and completion criteria for V1; does not authorize a new LLM call, full rerun, runtime change, score-sheet implementation, buyer-facing wording, D10 progress, or §13 sign-off.
- `Internal_Testing_Evidence_Discipline_Deep_Dive.md` - **Pre-§11 deep-dive draft. Discipline contract only — not §11, not §13, not D10 evidence, not implementation, not runtime code, not client-facing copy, not pricing approval, not an underwriter / broker / insurer / compliance / certification / coverage / premium / fraud-prevention claim.** Authored 2026-06-01 by Cursor under operator instruction. Foundation spec for the internal testing-evidence discipline that formalizes the pre-spec footing in `Testing_Score_Sheet_Schema.md` and `Testing_Plan_V1.md`. Locks the 13-column row contract (12 carry-forward columns plus new `track` column), closed `event_type` / `pass_fail` / `failure_type` / `recorded_by` / `track` enums, the single-canonical-ledger rule (per-track separate ledgers explicitly rejected), the candidate-vs-evidence boundary for future auto-derivation (candidates live under `audit_outputs/score_sheet_candidates/`; promoted candidate files renamed or archived, never deleted), the internal-only v1 disclosure boundary, 14 named failure modes (including PII / secrets / raw-payload contamination, silent candidate deletion, auto-promotion drift, per-track ledger split), and 12 audit requirements including a hard no-PII guard at §9.1. §10.A carries direction recorded by the 2026-06-01 7-axis stress test as verdict candidates (Q1 single canonical ledger plus required `track` column; Q4 closed list of four calibration-observation triggers; Q5 permanent retention for compliant rows plus PII guard; Q7 candidate-only auto-derivation; Q10 all 13 columns mandatory) for a future operator-authorized D-lockdown; §10.B carries still-open Q2/Q3/Q6/Q8/Q9/Q11/Q12/Q13; §10.C records the recommended Wave 0 → Wave 4 staged cadence (Wave 0 = tonight's draft + stress test, Wave 1 = candidate-emit deep-dive, Wave 2 = first emitter implementation, Wave 3 = interactive review script + PII pre-commit hook, Wave 4 = gradual emitter expansion). No D-decisions locked. §11 sign-off pending operator authorship. Paired 7-axis stress-test entry in `think_sheet.md` under "Sub-question stress test — Internal Testing Evidence Discipline §10 (2026-06-01)". Gate-clean as `audit_outputs/internal_testing_evidence_discipline_deep_dive_draft_20260601_20260602T041000Z.md` (packet size 96,729 bytes; touched files 2; blocking=0; warnings=0).
