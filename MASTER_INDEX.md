...
- `4. Product_Roadmap/Score_Sheet_Candidate_Review_Promotion_Deep_Dive.md` - **Draft pre-spec (Wave 3). Not §11 signed. Not implementation authorization.** Authored 2026-06-03. Defines human-in-the-loop candidate review and promotion discipline after signed Wave 1 candidate-emission contract and implemented Wave 2 emitter path (`audit_tools/pre_ship_audit.py` / `audit_tools/score_sheet_candidate_emit.py`). Locks: single holding pool `audit_outputs/score_sheet_candidates/`; hard 8-item operator checklist before promotion; `rejected/` archive with operator reason (no mutation-engine feed); manual-only promotion with `event_id` assigned at promotion (`TE-YYYYMMDD-<track_slug>-NNNN`); in-band textual proof (`candidate_packet_id`, `candidate_ref`, operator-bearing `recorded_by`); explicit forbidden automation list; Manus email-security research reviewed as context only (§2 non-adoption table). Does not authorize `review_ledger.py`, pre-commit hook, canonical ledger automation, crypto signatures, or client/compliance/insurance claims. 7-axis stress test recorded in `think_sheet.md` (2026-06-03 entry).
