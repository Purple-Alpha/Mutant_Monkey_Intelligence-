...
- 4. Product_Roadmap/Attachment_Risk_Agent_Design_Contract_Deep_Dive.md - **DRAFT 2026-06-08; not §11 signed; no runtime code authorized.** Candidate per-agent Agent Design Contract for swarm #30 Attachment Risk. Draft boundary: future `AttachmentRiskAgent` wraps immutable `score_attachment_risk` (`core/precursor/attachment_classifier.py`) as a Layer 2 Detection / Authority L3 / Stage A / Evidence Stage 1 Synthetic wrapper; emits facts-only attachment-risk indicator names as `observed_facts`; no numeric attachment risk score, classification, raw filename, hash, MIME value, content reference, extracted text, raw bytes, execution/detonation/extraction, network/sandbox/reputation lookup, default-registry registration, production dispatch, scoring/rubric change, or autonomous action. §11 signature remains operator-only before implementation.
