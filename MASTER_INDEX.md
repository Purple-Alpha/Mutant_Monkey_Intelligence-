...
- 4. Product_Roadmap/Next_Action_Decision_Rubric_Deep_Dive.md - **§11 SIGNED 2026-06-04 by Matt Nichol (Zebra-Comet); gate audit clean (grok-4, 0 blocking / 0 warnings, packet `3805524a…`, report `next_action_decision_rubric_signoff_20260604T062334Z.md`).** Spec-first contract for the tactical-layer Next-Action Decision Rubric — a deterministic 5-axis scoring engine (`leverage`, `risk_reduction`, `evidence_strength`, `future_cost` inverted, `reversibility`; 0-2 each, max 10) that ranks 3-7 next-action candidates inside an active session WITHOUT deciding among them (D2: rubric ranks, Matt selects, reality audits). Locks D1-D12 (rubric type / authority model / 5-axis structure / 3-7 option set / naming / mismatch-only calibration / pre-execution expectation / override path / one-action-per-cycle / equal weighting / interpretive bands / no autonomous rule mutation) plus D13-D19 from the §10 resolution: D13 hybrid option source (queue default + conversational when stale), D14 informal operator modes (stated, not enforced), D15 dedicated `decision_cycles_log.md` persistence, D16 calibration at 14-day retro + ≥3 PARTIAL/FAIL streak trigger, D17 Grok audit on FAIL only + operator opt-in, D18 compressed system-prompt deferred one trial cycle, D19 reference-scored "do nothing" on the same five axes. Distinct from `think_sheet.md` (strategic/idea-level) and `Client_Facing_5_Axis_Email_Scoring_Rubric_Deep_Dive.md` (buyer-facing/email-level); the label "5-axis rubric" is forbidden for it (naming-collision failure mode, D5).
...
- agent_concepts/Blue_Team_Swarm_70_Agent_Scoreboard.md - **WORKING SCOREBOARD, pre-spec, unsigned, NOT §11, authority-free (created 2026-06-07 by Cursor, execution lane).** Reconciles the 70-agent SPARK inventory and the 6-layer Design Tree against the live runtime into one table: all 70 agents with execution-lane-verified runtime status (`GOVERNED_AGENT` / `DETECTOR_FUNCTION` / `GOVERNANCE_DOC_ONLY` / `SPEC_ONLY` / `NOT_STARTED`), cited code paths where verified, DRAFT 6-layer placement, and Stage A/B. Headline finding: the swarm is currently a detector stack, not a governed swarm — exactly one governed agent (#10), ~28 detector functions, and the orchestration spine (#1 Commander case loop, #2 Mission Context) is the critical gap. Builds/authorizes nothing (Rule 4: a row is status, never build permission). Layer reconciliation + canonical naming + build order are flagged as open questions ROUTED TO CLAUDE per AGENTS §2.1.2 (design/pattern-reconciliation lane), not finalized by the execution lane.
